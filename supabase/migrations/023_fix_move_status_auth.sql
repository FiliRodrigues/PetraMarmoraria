-- 023_fix_move_status_auth.sql
-- move_service_order_status é SECURITY DEFINER e bypassa RLS.
-- Adiciona guarda interna: só admin ou vendedor autenticados podem mover OS.
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE id = auth.uid()
      AND active = true
      AND (roles && ARRAY['admin','vendedor']::text[])
  ) THEN
    RAISE EXCEPTION 'Apenas administradores ou vendedores podem mover ordens de serviço';
  END IF;
